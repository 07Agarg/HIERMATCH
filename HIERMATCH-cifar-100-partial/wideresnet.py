...
class model_bn_2(nn.Module):
    def __init__(self, model, feature_size=512, classes=100):
        ...
        self.classifier_1 = nn.Sequential(
            nn.Linear(feature_size , classes[1]),
        )

        self.classifier_2 = nn.Sequential(
            nn.Linear(feature_size // 2, classes[0]),
        )
    ...
    def forward(self, x):
        ...
        x_1 =  x[:, :self.feature_size//2]
        x_2 =  x[:, self.feature_size//2:]
        
        order_input  = torch.cat([x_1, x_2.detach()], 1)
        family_input = x_2
